...
public class ComputerBuilding {
    public ComputerBuilding(){
        ...
        RM.DidYouKnow.addFakeRecipe(F, ST.array(
                ...
                ItemList.fakeItemCPUWafer.get(1)
                , ItemList.fakeItemSiliconCPUWaferDeveloped.get(1)
                , ItemList.fakeItemSiliconCPUWaferHardBaked.get(1)
                , ItemList.fakeItemSiliconCPUWaferDoped.get(1)
                , ItemList.fakeItemSiliconCPUWaferChecked.get(1)
                , ItemList.fakeItemCPUDie.get(1)
        ), null, ZL_LONG, FL.array(FL.Sap_Rainbow.make(250)),ZL_FS, 0, 0, 0);

        recipeManager.ElectronicsDesigner.addRecipeX(T,32,36000, ST.array(IL.Circuit_Good.get(0),
                gRegistry.getItem(10102,0),gRegistry.getItem(10112,0), OP.plate.mat(MT.SiO2,4)),
                FL.array(ZL_FS),FL.array(ZL_FS),ItemList.CPUPhotomask200um.get(1));

        recipeManager.LaserCutter.addRecipeX(T,96,2000, ST.array(OP.bouleGt.mat(MT.Si, 1)),FL.array(ZL_FS),FL.array(ZL_FS), ItemList.SiliconPlate8inchTier1.get(1));
