...
public interface IMappedStructure extends ITileEntityMultiBlockController {
    ...
    @Nullable
     ...
     int getDesign(int mapX,int mapY,int mapZ);
    ...
    default void debugMappedStructure(ChunkCoordinates lastFailedPos, int machineX, int machineY, int machineZ, int xMapOffset, int yMapOffset, int zMapOffset){
        if (lastFailedPos != null) FxRenderBlockOutline.removeBlockOutlineToRender(lastFailedPos);
        int tX = getX(), tY = getY(), tZ = getZ();
        if (!getWorld().blockExists(tX, tY, tZ)) return;
        tX = utils.getRealX(getFacing(), tX, xMapOffset, -zMapOffset);
        tY += yMapOffset;
        tZ = utils.getRealZ(getFacing(), tZ, xMapOffset, -zMapOffset);
        int mapX, mapY, mapZ;
        for (mapY = 0; mapY < machineY; mapY++) for (mapZ = 0; mapZ < machineZ ; mapZ++) for (mapX = 0; mapX < machineX; mapX++) {
            ...
            if (isIgnored(mapX,mapY,mapZ)) continue;
            if (utils.checkAndSetTarget(this, realX, realY, realZ, getBlockID(mapX, mapY, mapZ), getRegistryID(mapX,mapY,mapZ), getDesign(mapX,mapY,mapZ), getUsage(mapX,mapY,mapZ))) {
                if (WD.te(getWorld(),new ChunkCoordinates(realX,realY,realZ),false) instanceof IComputeNode) getComputeNodesCoordList().add(new ChunkCoordinates(realX,realY,realZ));
            } else getWorld().setBlock(realX,realY,realZ, Blocks.glass);
        }
    }
}
